...
fit_2 = MCMC_fit.MCMCfit(usbl)

fit_2.model_parameters_guess = [2.45935341e+06, 1.48791735e-01, 2.65444058e+00, -2.62081329e+00, -1.63126084e-01, -8.10868845e-02, -3.30072563e-01, 0, 0]

fit_2.fit_parameters['t0'][1] = [2459325.00, 2459375.00] # t0 limits, changed
fit_2.fit_parameters['u0'][1] = [-0.55, 0.55] # u0 limits, computed and changed
fit_2.fit_parameters['log_tE'][1] = [1.0,3.0] # logtE limits in days, changed from log(20) - log(150)

fit_2.fit_parameters['log_rho'][1] = [-3, -2]
fit_2.fit_parameters['log_separation'][1] = [-1.1,1.1] # log_s limits,
...
if perform_long_fit == True:
    ...
    # Save it
    np.save('results/noZTF_USBL+MCMC_parallax.npy', fit_2.fit_results['MCMC_chains'])

else:
    # Use the precomputed Differential Evolution (DE) results:
    fit_2.fit_results['MCMC_chains'] = np.load('results/noZTF_USBL+MCMC_parallax.npy')
    fit_2.fit_results['best_model'] = fit_2.fit_results['MCMC_chains'][346501][0:-1]
