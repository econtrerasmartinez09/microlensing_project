...
# for example, exported ATLAS .txt file that was generated after queuing it from their server

# Replace 'job540091.txt' with your exported ATLAS data file
atlas = np.loadtxt('job540091.txt',dtype=np.ndarray,usecols=(0,1,2,5))

# Note: ATLAS will most likely need to be sorted by filter code; in my case, by c and o filter code

c_filter = np.array((0,0,0))   # creating empty 1x3 array; change or add in case of different filter codes
o_filter = np.array((0,0,0))

for i in range(len(atlas)):
    if atlas[i,3] == 'c':
        c_filter = np.vstack((c_filter,atlas[i,0:3]))   # vertically stacking the data (MJD), mag, mag_err
    if atlas[i,3] == 'o':
        o_filter = np.vstack((o_filter, atlas[i,0:3]))   # change or add additional in case of different filter codes

c_filter = np.delete(c_filter,(0),axis=0)   # deleting the first row of the array since they are empty
o_filter = np.delete(o_filter,(0),axis=0)

for i in range(len(c_filter)):   # checking to see if mag is negative; do not need
    if c_filter[i,1] < 0:
        c_filter = np.delete(c_filter,(i),axis=0)

for i in range(len(o_filter)):
    if o_filter[i,1] < 0:
        o_filter = np.delete(o_filter,(i),axis=0)

# Will need to extract the  year from all files in isot format; necessary parameter for HJD conversion func

c_filter_mjd = c_filter[:,0]
t0 = Time(c_filter_mjd,format='mjd')
atlas_c_isot = t0.isot

o_filter_mjd = o_filter[:,0]
t1 =- Time(o_filter_mjd,format='mjd')
atlas_o_isot = t1.isot

atlas_c_year = np.array((0))

for i in range(len(atlas_c_isot)):
    atlas_c_year = np.vstack((atlas_c_year,int(atlas_o_isot[i][0:4])))

atlas_c_year = np.delete(atlas_c_year,(0),axis=0)   # array containing the year parameter

atlas_o_year = np.array((0))

for i in range(len(atlas_o_isot)):
    atlas_o_year = np.vstack((atlas_o_year,int(atlas_o_isot[i][0:4])))

atlas_o_year = np.delete(atlas_o_year,(0),axis=0)   # array containing the year parameter

# Now calculating the HJD for all

atlas_c_hjd = np.array((0))
#year = atlas_c_isot   # check if i even use it

for i in range(len(atlas_c_year)):
    atlas_c_hjd = np.vstack((atlas_c_hjd,calc_hjd(c_filter_mjd[i],atlas_c_year[i])))

atlas_c_hjd = np.delete(atlas_c_hjd,(0),axis=0)   # a column array of the HJD

c_filter = np.delete(c_filter,0,axis=1)   # deleting the mjd column of the c filter array
c_filter = np.concatenate((atlas_c_hjd,c_filter),axis=1)   # concatenating the hjd column as first column, final array

atlas_o_hjd = np.array((0))

#year = atlas_o_isot

for i in range(len(atlas_o_year)):
    atlas_o_hjd = np.vstack((atlas_o_hjd,calc_hjd(o_filter_mjd[i],atlas_o_year[i])))

atlas_o_hjd = np.delete(atlas_o_hjd,(0),axis=0)

o_filter = np.delete(o_filter,0,axis=1)
o_filter = np.concatenate((atlas_o_hjd,o_filter),axis=1)   # final array

np.savetxt('reduced_atlas_c.dat',c_filter,fmt='%s')   # saving the data array as a .dat file for import
np.savetxt('reduced_atlas_o.dat',o_filter,fmt='%s')   # Note: will need to use the JD to HJD script to convert the data (Crucial)
