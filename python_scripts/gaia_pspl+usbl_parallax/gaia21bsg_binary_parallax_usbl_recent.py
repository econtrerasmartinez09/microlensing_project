...
usbl = USBL_model.USBLmodel(your_event,fancy_parameters=fancy,parallax=['Full',2459352.258862019])
fit_2 = DE_fit.DEfit(usbl, telescopes_fluxes_method='polyfit', DE_population_size=10, max_iteration=10000, display_progress=True)
fit_2.model_parameter_guess = [2.45935226e+06, -1.48194554e-01, 2.29183432e+00, -2.19550744e+00, 2.08112135e-01, -2.32378344e-01, -9.72711533e-01, 0, 0] # [t0, u0, log_tE, log_rho, log_s, log_q, alpha]
...
fit_2.fit_parameters['log_tE'][1] = [0,3] # logtE limits in days, changed from log(20) - log(150)

fit_2.fit_parameters['log_rho'][1] = [-6.042577190325831, -1]
fit_2.fit_parameters['log_separation'][1] = [-1, 1] # log_s limits,
...
if perform_long_fit == True:
    ...
    # Save it
    np.save('noZTF_USBL_parallax_gaia21bsg.npy', fit_2.fit_results['DE_population'])

else:
    # Use the precomputed Differential Evolution (DE) results:
    fit_2.fit_results['DE_population'] = np.load('noZTF_USBL_noparallax_gaia21bsg.npy')
